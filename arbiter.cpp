...
void arbiter :: func()
{
	...
	{ 
		...
		{	
			...
			if(v_id[0] < req0.read()[0]) v_req[0]=3; // east //0110 0101
			else {	
				if(v_id[0] > req0.read()[0])v_req[0]=5; //west
				else{
					if(v_id[1] < req0.read()[1])v_req[0]=3; // east
					else{
						if(v_id[1] > req0.read()[1])v_req[0]=5; //west
						else{ 
							 if(v_id[2] < req0.read()[2])v_req[0]=4; //south
							 else{	
								 if(v_id[2] > req0.read()[2])v_req[0]=2; //north
								 else{
									 if(v_id[3] < req0.read()[3]) v_req[0]=4;//south
									 else{
										 if(v_id[3] > req0.read()[3])v_req[0]=2; //north
										 else v_req[0]=1; // destination
									}
								}
							}
						}
		...
		if (!req1.read()[4]) //if buffer is not empty 
		{
			...
			if(v_id[0] < req1.read()[0]) v_req[1]=3; //east
			else {	
				if(v_id[0] > req1.read()[0])v_req[1]=5; //west
				else{
					if(v_id[1] < req1.read()[1])v_req[1]=3; //east
					else{
						if(v_id[1] > req1.read()[1])v_req[1]=5; //west
						else{ 
							 if(v_id[2] < req1.read()[2])v_req[1]=4; //south	
							 else{	
								 if(v_id[2] > req1.read()[2])v_req[1]=2; //north
								 else{
									 if(v_id[3] < req1.read()[3]) v_req[1]=4; //south
									 else{
										 if(v_id[3] > req1.read()[3])v_req[1]=2; //north
										 else v_req[1]=1; 
									}
								}
							}
		...
		if (!req2.read()[4]) //if buffer is not empty 
		{
			...
			if(v_id[0] < req2.read()[0]) v_req[2]=3; //east
			else {	
				if(v_id[0] > req2.read()[0])v_req[2]=5; //west
				else{
					if(v_id[1] < req2.read()[1])v_req[2]=3; //east
					else{
						if(v_id[1] > req2.read()[1])v_req[2]=5; //west
						else{ 
							 if(v_id[2] < req2.read()[2])v_req[2]=4; //south	
							 else{	
								 if(v_id[2] > req2.read()[2])v_req[2]=2; //north
								 else{
									 if(v_id[3] < req2.read()[3]) v_req[2]=4; //south
									 else{
										 if(v_id[3] > req2.read()[3])v_req[2]=2; //north
										 else v_req[2]=1; //destination
									}
								}
							}
						}
		...
		if (!req3.read()[4]) //if buffer is not empty 
		{
			...
			if(v_id[0] < req3.read()[0]) v_req[3]=3; //east 
			else {	
				if(v_id[0] > req3.read()[0])v_req[3]=5; //west
				else{
					if(v_id[1] < req3.read()[1])v_req[3]=3; //east
					else{
						if(v_id[1] > req3.read()[1])v_req[3]=5; //west
						else{ 
							 if(v_id[2] < req3.read()[2])v_req[3]=4; //south
							 else{	
								 if(v_id[2] > req3.read()[2])v_req[3]=2; //north
								 else{
									 if(v_id[3] < req3.read()[3]) v_req[3]=4;//south
									 else{
										 if(v_id[3] > req3.read()[3])v_req[3]=2; //north
		...
		if (!req4.read()[4]) //if buffer is not empty  
		{
			...
			if(v_id[0] < req4.read()[0]) v_req[4]=3; //east //0110 0101
			else {	
				if(v_id[0] > req4.read()[0])v_req[4]=5; //west
				else{
					if(v_id[1] < req4.read()[1])v_req[4]=3; //east
					else{
						if(v_id[1] > req4.read()[1])v_req[4]=5; //west
						else{ 
							 if(v_id[2] < req4.read()[2])v_req[4]=4; //south	
							 else{	
								 if(v_id[2] > req4.read()[2])v_req[4]=2; //north
								 else{
									 if(v_id[3] < req4.read()[3]) v_req[4]=4;//south
									 else{
										 if(v_id[3] > req0.read()[3])v_req[4]=2; //north
										 else v_req[4]=1; //destination
									}
								}
							}
						}
